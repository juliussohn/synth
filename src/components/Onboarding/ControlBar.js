...
class ArrowButton extends React.Component {
  ...
  render() {
    ...
    return <ArrowButtonContainer onHoverEnd={this.onHoverEnd} onTapStart={this.onTapStart} onHoverStart={this.onHoverStart} onTap={this.onTap} transition={{ damping: 300, type: "spring" }} initial="rest" >
      <svg width="54" height="54" viewBox="0 0 54 54" fill="none"  >
        <circle cx="27" cy="27" r="26.5" stroke="grey" />
        <motion.circle transition={{ damping: 300, type: "spring", mass: 2 }} animate={state} variants={circleMotion} cx="27" cy="27" r="26.5" stroke="white" />
        <motion.path transition={{ damping: 300, type: "spring", mass: 2 }} animate={state} variants={arrowMotion} d="M32.7895 21.12L31.1395 22.74L34.5895 26.52H16.4395V28.77H34.5895L31.1395 32.55L32.7895 34.17L38.6995 27.63L32.7895 21.12Z" fill="#ffffff" />
      </svg>
...
class ControlBar extends React.Component {
  ...
  renderCounter() {
    ...
    const { prevStep, animating } = this.state
      ...
      <CurrentPageContainer>
        ...
        }
        {
          animating && <Page
            ...
            initial={{ y: `100%` }}
            animate={{ y: 0 }}
            onAnimationComplete={() => { this.setState({ animating: false }) }}
            transition={transition}
          >
            {currentStep + 1}</Page>
        }
