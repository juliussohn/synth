...
class AnimatedText extends React.Component {
  ...
  isCurrent() {
    ...
    return step == currentStep && this.state.show;
  }


  componentDidUpdate(prevProps) {
    const { step, currentStep, animating } = this.props
    //when prev animation finished
    if (prevProps.animating != animating && !animating && step == currentStep) {
      this.show()
    }
  }
  ...
  show() {
    this.setState({ show: true }, () => {
      //Headline
      const headlineHeight = this.headlineContainer.getBoundingClientRect().height
      const headlineBarCount = Math.ceil(headlineHeight / (bar.headline + bar.spacing + 0.5 * bar.spacing));

      //Text
      const paragraphHeight = this.paragraphContainer.getBoundingClientRect().height
      const paragraphBarCount = Math.ceil(paragraphHeight / (bar.paragraph + bar.spacing + 0.5 * bar.spacing));
  ...
  setBars() {
    ...
    const barCount = Math.ceil(containerHeight / (barHeight + bar.spacing + 0.5 * bar.spacing));
  ...
  getBars(height, count, callback) {
    ...
    return [...Array(count)].map((b, i) => {
      ...
      return <Bar
        ...
        onAnimationComplete={() => { finished(i) }}>
  ...
  getImages() {
    ...
    if (!images) return
    return images.map((item, i) => <Image
    key={i}
      ...
      onAnimationComplete={() => { this.animationComplete('images') }}>
  ...
  render() {
    ...
    return <Container >
      ...
      <HeadlineContainer ref={(el) => { this.headlineContainer = el }}>
        <AnimatePresence >
          {this.isCurrent() && this.getBars(bar.headline, headlineBarCount, () => { this.animationComplete('headline') })}
          {this.isCurrent() && this.getText(headline, Headline, headlineBarCount)}
        </AnimatePresence>

      </HeadlineContainer>
      <ParagraphContainer ref={(el) => { this.paragraphContainer = el }}>
        <AnimatePresence >
          {this.isCurrent() && this.getBars(bar.paragraph, paragraphBarCount, () => { this.animationComplete('paragraph') })}
          {this.isCurrent() && this.getText(children, Paragraph, paragraphBarCount)}
        </AnimatePresence>
